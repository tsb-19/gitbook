...
Equivalently, Let y ⊆ Y be a strategy of Player II. Then, x ⊆ X is called a Best Response (BR) to y if A(x, y)  \geq  A (x’, y) for any x’ ⊆ X.
...
X\* = $$\{ \mathbf p$$ = $$(p_1, . . . , p_m )^T$$ : $$p_i$$ \geq 0, for i = 1, . . . , m and $$p_1 +...+ p_m$$ =1$$\}$$
Y\* = $$\{ \mathbf q$$ = $$(q_1, . . . , q_n)^T$$ : $$q_j$$ \geq 0, for j = 1, . . . , n and $$q_1 +...+ q_n$$ =1$$\}$$
> 
...
Suppose that for i = 1, 2, ... , n, Player i uses $$\mathbf p_i = (p_{i1}, p_{i2}, ... , p_{im_i}) \in X^*_i$$. Then the average payoff to player j is $$u_j(\mathbf p_1,...,\mathbf p_n)=\sum_{i_1}...\sum_{i_n}p_{1i_1}...p_{ni_n}u_j(i_1,...,i_n)$$
> 
...
> In general, let [A, B] be a an m×n bimatrix game where A=($$a_{ij}$$), B=($$b_{ij}$$). A probability distribution of the outcomes of this game is given by $$p_{ij}, p_{ij}\geq0, \sum_{i,j}p_{ij}=1$$. This means that the instruction of playing (Row i, Column j) to the players with probability $$p_{ij}$$. This probability distribution is called a Correlated Equilibrium (CE) if it is for the best interest for players to comply with the instruction.
> 

> Inequalities characterizing CE: $$Prob(Column\ j\ for\ II | Row\ i\ for\ I) = \frac{Prob(Column\ j\ for\ II, Row\ i\ for\ I)}{Prob(Row\ i\ for\ I)} =\frac{p_{ij}}{\sum_k p_{ik}}$$.
For Player I to comply with the instruction to play Row i, the payoff must be better than deviating the instruction in playing Row r instead. This means $$\sum_j\frac{p_{ij}}{\sum_k p_{ik}}a_{ij} \geq \sum_j\frac{p_{ij}}{\sum_k p_{ik}}a_{rj}$$, for r =1,...,m.
Equivalently, $$\sum_j p_{ij} (a_{ij} - a_{rj})\geq0$$, for r=1,...,m.

> Definition of CE: A probability distribution $$p_{ij}$$ over the outcomes of the m×n bimatrix game [A, B] is called a Correlated Equilibrium whenever $$\sum_j p_{ij} (a_{ij} - a_{rj})\geq0$$ for i, r=1,...m and $$\sum_i p_{ij} (b_{ij} - b_{is})\geq0$$ for j, s=1,...n.
> 
